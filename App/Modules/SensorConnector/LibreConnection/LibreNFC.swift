...
import Foundation
    ...
    extension LibrePairingError: CustomStringConvertible {
        var description: String {
            switch self {
            case let .invalidPatchInfo(patchInfo: patchInfo):
                return "Invalid patchInfo: \(patchInfo)"

            case let .unsupportedSensor(type: type):
    ...
    class LibreNFC: NSObject {
        func readSensor(enableStreaming _: Bool) async throws -> LibrePairingResult {
